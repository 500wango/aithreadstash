(function() {
    ...
    function getCleanTitle(messages) {
        let t = (document.title || '').trim();
        
        // 从页面标题中提取会话标题，去除DeepSeek相关后缀
        if (t) {
            // 移除常见的DeepSeek页面后缀
            t = t.replace(/\s*[-–—]\s*DeepSeek.*$/i, '')
                 .replace(/\s*\|\s*DeepSeek.*$/i, '')
                 .replace(/\s*DeepSeek\s*$/i, '')
                 .trim();
        }
        
        // 过滤无效标题
        const invalidTitles = ['DeepSeek', 'AI Assistant', 'Chat', ''];
        if (!t || invalidTitles.includes(t)) {
            // 回退到第一条用户消息的片段
            if (messages && Array.isArray(messages)) {
                const firstUserMsg = messages.find(m => m.role === 'user' || m.author === 'User');
                if (firstUserMsg && firstUserMsg.content) {
                    t = firstUserMsg.content.substring(0, 50).trim();
                    if (t.length === 50) t += '...';
                }
            }
        }
    ...
    if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.onMessage) {
        chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
            if (request.action === 'ping') {
                ...
                sendResponse({ success: true, message: 'DeepSeek content script ready' });
                return true;
            }
            ...
            // --- Align with background.js trigger: 'export-deepseek-content' ---
            if (request.action === 'export-deepseek-content') {
                console.log('[AI-ThreadStash][DeepSeek] Received export-deepseek-content trigger');
                try {
                    const conversation = parseDeepSeekConversationClean();

                    if (conversation && !conversation.error && Array.isArray(conversation.turns) && conversation.turns.length > 0) {
                        // Map to preview payload structure { title, messages }
                        const payload = (function mapConversationToPreviewPayload(conv) {
                            const messages = (conv.turns || []).map(t => {
                                const role = t.role === 'assistant' ? 'DeepSeek' : 'User';
                                let content = '';
                                if (t && t.content) {
                                    if (typeof t.content === 'object') {
                                        // 优先使用清理后的 HTML，以最大化保留列表/代码块等格式
                                        if (t.content.html && String(t.content.html).trim().length > 0) {
                                            content = String(t.content.html);
                                        } else if (t.content.text) {
                                            content = String(t.content.text);
                                        }
                            ...
                            const title = conv.title || getCleanTitle(messages);
                        ...
                        chrome.runtime.sendMessage({ action: 'deepseek-content-ready', data: payload }, () => {
                            if (chrome.runtime.lastError) {
                                console.warn('[AI-ThreadStash][DeepSeek] deepseek-content-ready sendMessage error:', chrome.runtime.lastError.message);
                            } else {
                                console.log('[AI-ThreadStash][DeepSeek] deepseek-content-ready sent. messages:', payload.messages.length);
                            }
                        });
                    } else {
                        const err = conversation?.error || 'No conversation content found';
                        try { sendResponse({ success: false, message: err }); } catch(_) {}
                        chrome.runtime.sendMessage({ action: 'deepseek-content-failed', error: err }, () => {
                            if (chrome.runtime.lastError) {
                                console.warn('[AI-ThreadStash][DeepSeek] deepseek-content-failed sendMessage error:', chrome.runtime.lastError.message);
                            }
                        });
                    }
                } catch (e) {
                    ...
                    chrome.runtime.sendMessage({ action: 'deepseek-content-failed', error: errMsg }, () => {
                        if (chrome.runtime.lastError) {
                            console.warn('[AI-ThreadStash][DeepSeek] deepseek-content-failed sendMessage error:', chrome.runtime.lastError.message);
                        }
                    });
