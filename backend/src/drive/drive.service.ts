...
export class DriveService {
  ...
  async connect(userId: number, accessToken: string, refreshToken?: string, folderId?: string, folderName?: string) {
    if (process.env.NODE_ENV === 'development') {
      ...
      console.log('[DriveService] connect()', { userId, hasAccessToken: Boolean(accessToken), folderId, folderName, beforeSize: this.driveState.size });
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      if (process.env.NODE_ENV === 'development') {
        // eslint-disable-next-line no-console
        console.log('[DriveService] connect() set done', { userId, afterSize: this.driveState.size });
      }
      return;
    }
  ...
  async status(userId: number) {
    if (process.env.NODE_ENV === 'development') {
      ...
      if (process.env.NODE_ENV === 'development') {
        // eslint-disable-next-line no-console
        console.log('[DriveService] status()', { userId, hasState: Boolean(state), keys: state ? Object.keys(state) : [], size: this.driveState.size });
      }
  ...
  async selectFolder(userId: number, folderId: string, folderName?: string) {
    if (process.env.NODE_ENV === 'development') {
      ...
      if (process.env.NODE_ENV === 'development') {
        // eslint-disable-next-line no-console
        console.log('[DriveService] selectFolder()', { userId, folderId, folderName });
      }
      return;
    }
  ...
  async disconnect(userId: number) {
    if (process.env.NODE_ENV === 'development') {
      ...
      if (process.env.NODE_ENV === 'development') {
        // eslint-disable-next-line no-console
        console.log('[DriveService] disconnect()', { userId, afterSize: this.driveState.size });
      }
      return;
    }
  ...
  async save(userId: number, data: { title: string; content: string; summary?: string; tags?: string }): Promise<string> {
    ...
    if (process.env.NODE_ENV === 'development') {
      // eslint-disable-next-line no-console
      console.log('[DriveService] save()', { userId, hasState: Boolean(state), state, size: this.driveState.size });
    }
